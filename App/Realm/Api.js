...
const getUUID = () => Math.random().toString(36).substring(2) + (new Date()).getTime().toString(36)
...
export const getRealmSettings = ImmutableRealm((props, realm) => {
    ...
    const offlineAvailableFiles = files ? files.filtered(`isAvailable == true`).length : 0
    if (!settings) {
